...
def login(request):
    if (request.method == 'POST'):
        ...
        u_psw = User.objects.filter(uname=username).first()
        print(u_psw.upsw)
        ...
        if u_phone.count()|u_email.count()|u_name.count() == 0:
            ...
            if u_psw.upsw != password:
                print(f'upsw{u_psw.upsw}')
                print(f'password{password}')
                # 密码错误
                context = {'error_name': 0, 'error_pwd': 1, 'username': username, 'password': password}
                return render(request, 'login.html', context)
            else:
                thisuser = User.objects.filter(uname=username)
                request.session['user'] = {'islogin': True,
                                           'username': thisuser.first().uname,
                                           'email': thisuser.first().uemail,
                                           'phone': thisuser.first().uphone,
...
def register(request):
    if request.method =="POST":
        ...
        newuser = User(uname=username, uemail=email, upsw=password,uphone=phone,usex=sex,uid=uid,uadmin=0)
        newuser.save()
        return HttpResponseRedirect('/user/login')
...
def sent_email_captcha(email,random):
    my_sender = '3086848266@qq.com'  # 发件人邮箱账号
    my_pass = 'qqxy991030'  # 发件人邮箱密码
...
def editpassword(request):
    ...
    if request.method == 'POST':
        newpassword = request.POST.get('newpassword')
        print(newpassword) 
        User.objects.filter(uid=userinfo['uid']).update(upsw=newpassword)
        context = {'success': '修改成功！', 'newpassword': newpassword ,'userinfo': userinfo }
        print(userinfo)
        return render(request,'editpwd.html',context)
    else:
        context = {'error_pwd': 0, 'captcha': '', 'newpassword': '', 'userinfo': userinfo}
